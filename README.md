...
**v0 (MVP-zero) 완료** — KOSPI 200 지분 그래프 + Cypher 쿼리 3종 + Streamlit
워크벤치까지 가동.

- ✅ KOSPI 200 회사 노드 200개 + DART 사업보고서 기반 OWNS 엣지 236건
  (양방향 — 타법인 출자 + 최대주주 — 추출 후 Pydantic 검증·중복 제거)
- ✅ 그래프 읽기 쿼리 3종 — `get_subsidiaries`, `find_common_parents`,
  `get_within_2hop`
- ✅ Streamlit 워크벤치 — 사이드바 워크플로우 선택 + 종목 selectbox + 결과 표
  + Plotly 그래프 시각화
- ✅ 모든 결과에 출처(DART 공시번호 `source_id`) 동반
- ✅ pytest -m "not e2e" 149 통과, ruff·mypy 0 error
- 🔜 v0.5 — universe를 KOSPI 200 → 전체 KOSPI + KOSDAQ로 확장 (매칭 실패의
  (A) 종류 — 다른 상장사라서 매칭 실패한 후보 — 즉시 해소)
