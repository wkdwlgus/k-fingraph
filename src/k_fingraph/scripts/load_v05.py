"""v0.5 ingestion entrypoint — extended universe (KOSPI 보통주 + KOSDAQ 보통주)
+ DART OWNS into a Neo4j instance.

Run:
    uv run python -m k_fingraph.scripts.load_v05
...
The drop classification re-uses ADR 0007's vocabulary; under v0.5 (A) is the
bucket that universe expansion is designed to recover, so its delta vs the
v0 baseline (A 338 / B-1 2,948 / B-2 6,964) is the primary acceptance signal.

`load_v0.py` remains in place as the v0 historical artifact (paired with
`data/processed/v0_load/report.json`); this script is the current entry point.

Network footprint: 1 corpCode reuse (cached at data/raw/dart/corp_codes/) +
N forward + N reverse DART calls where N ≈ 2,659. Spaced by SLEEP_BETWEEN_CALLS
to stay under DART's per-IP burst limit. Single-call failures are logged and
the run continues.
"""
...
from k_fingraph.schemas.universe import Market
...
DEFAULT_KOSPI_CSV = PROJECT_ROOT / "data" / "reference" / "kospi_all.csv"
DEFAULT_KOSDAQ_CSV = PROJECT_ROOT / "data" / "reference" / "kosdaq_all.csv"
DEFAULT_CORP_CODE_DIR = PROJECT_ROOT / "data" / "raw" / "dart" / "corp_codes"
DEFAULT_CORP_CODE_XML = DEFAULT_CORP_CODE_DIR / "CORPCODE.xml"
DEFAULT_REPORT_PATH = PROJECT_ROOT / "data" / "processed" / "v05_load" / "report.json"

# DART OpenAPI tolerates ~200 calls/min per IP. 0.5s = 120 calls/min, leaving
# headroom. ~2,659 companies × 2 endpoints × 0.5s ≈ 45 min total.
SLEEP_BETWEEN_CALLS = 0.5

DEFAULT_BSNS_YEAR = "2024"
REPRT_CODE_ANNUAL = "11011"

MatchedRow = tuple[str, str, str, Market]
...
def _build_companies(matched: list[MatchedRow], now: datetime) -> list[Company]:
    """v0.5 carries `market` from the KRX source instead of hardcoding it."""
    companies: list[Company] = []
    for ticker, corp_code, name, market in matched:
        companies.append(
            Company(
                ticker=ticker,
                corp_code=corp_code,
...
def _fetch_all_candidates(
    ...
    for i, (_ticker, corp_code, name, _market) in enumerate(matched, start=1):
        try:
...
def _build_report(
    *,
    ...
    return {
        "universe": {
            "kospi_count": universe_kospi,
            "kosdaq_count": universe_kosdaq,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="v0.5 extended universe + OWNS ingestion")
    parser.add_argument(
        "--kospi-csv",
        type=Path,
        default=DEFAULT_KOSPI_CSV,
        help="Path to KOSPI universe CSV (ticker,name)",
    )
    parser.add_argument(
        "--kosdaq-csv",
        type=Path,
        default=DEFAULT_KOSDAQ_CSV,
        help="Path to KOSDAQ universe CSV (ticker,name)",
    )
    parser.add_argument(
        "--corp-code-cache",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Where to write the JSON report (also printed to stdout)",
    )
    ...
    constituents = load_extended_universe(args.kospi_csv, args.kosdaq_csv)
    universe_kospi = sum(1 for c in constituents if c.market == "KOSPI")
    universe_kosdaq = sum(1 for c in constituents if c.market == "KOSDAQ")
    ...
    matched: list[MatchedRow] = [
        (m.ticker, m.corp_code, m.name, m.market) for m in memberships if m.corp_code is not None
    ]
    if args.limit is not None:
        matched = matched[: args.limit]
    unmatched_count = sum(1 for m in memberships if m.corp_code is None)
    universe = {corp_code for _, corp_code, _, _ in matched}
    ...
    report = _build_report(
        universe_kospi=universe_kospi,
        universe_kosdaq=universe_kosdaq,
