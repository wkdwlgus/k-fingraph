...
7. `docs/decisions/README.md` — ADR 인덱스 (제목·Status·재검토 트리거 표).
   개별 ADR은 인덱스에서 진입. 결정 변경은 새 ADR로 — 옛 ADR 본문은 절대 수정 금지
...
새 작업 들어갈 때는 `/plan` 명령 사용. 작업 단위는 한 plan 사이클로 끝낼 수
있는 크기로 쪼갠다. 바로 코딩 들어가지 않는다. 항상 plan → user confirm →
execute 순서.
...
1. `tasks/backlog.md`의 해당 sprint 섹션을 펼침 — 작업 단위 + "진입 전 결정
   재검토" bullet + "진입 전 ADR 작성" bullet을 모두 본다
